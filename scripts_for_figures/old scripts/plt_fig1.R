...
fig1 <- loadRData("~/Documents/GitHub/BNPconsistency/saves_for_figures/cmp_fig1.RData")
...
p = ggplot(fig1_, aes(x=K, y = pkn, colour = fig1_$Process_type)) +
  ...
  ylab('') + ggtitle(TeX(sprintf('Posterior dist for the number of clusters for $\\alpha =(%.3f, %.2f, %.2f)$,$\\N =%2.f$, \\hat{R} =(%.4f,%.4f,%.4f)',fig1$alpha[1],fig1$alpha[(max(fig1$K)+1)],fig1$alpha[(2*max(fig1$K)+1)],fig1$N[1],fig1$Rh[1],fig1$Rh[(max(fig1$K)+1)],fig1$Rh[(2*max(fig1$K)+1)])))+
...
pdf(file="~/Documents/GitHub/BNPconsistency/figures/Figure1.pdf")
plot(p)
dev.off()
