...
julia <- julia_setup()
#julia_install_package_if_needed("https://github.com/konkam/GibbsTypePriors")
julia_library("GibbsTypePriors")
julia_exists("Pkn_Dirichlet_mult")

df_prior = tibble(K= 1:10, 
                  Pkn_1 = round(julia_eval("Pkn_Dirichlet_mult.(1:10,20, 10, 2.0*10)"),3),
                  Pkn_2= round(julia_eval("Pkn_Dirichlet_mult.(1:10,200, 10, 2.0*10)"),3), 
                  Pkn_3 = round(julia_eval("Pkn_Dirichlet_mult.(1:10,2000, 10, 2.0*10)"),3),
                  Pkn_4 = round(julia_eval("Pkn_Dirichlet_mult.(1:10,20000, 10, 2.0*10)"),3))%>% gather(Process_type, pkn,Pkn_1:Pkn_4)

df_prior$Type =rep("Prior", dim(df_prior)[1])
fig8_$Type= rep("Posterior", dim(fig8_)[1])

df_merged = rbind(df_prior,fig8_[, c("K","Process_type", "pkn", "Type")] )

pkn.labs<- c("Pkn_1","Pkn_2","Pkn_3","Pkn_4")
names(pkn.labs) <- c(paste0("N = ", fig8_$N[1]),paste0("N = ", fig8_$N[max(fig8_$K)+1]),paste0("N = ", fig8_$N[(2*max(fig8_$K)+1)]),paste0("N = ", fig8_$N[(3*max(fig8_$K)+1)]))
pkn_names <- as_labeller(
  c(`Pkn_1` = paste0("N = ", fig8_$N[1]), `Pkn_2` = paste0("N = ", fig8_$N[max(fig8_$K)+1]),`Pkn_3` = paste0("N = ", fig8_$N[(2*max(fig8_$K)+1)]),`Pkn_4` = paste0("N = ", fig8_$N[(3*max(fig8_$K)+1)])))

p <- ggplot(df_merged, aes(K,pkn,color =Process_type))+geom_bar(aes(linetype=Type),size = 0.7, stat="identity",alpha =0, position = "identity")+
  geom_vline(xintercept=3,  linetype="dashed")+ylab("Density")+xlab(TeX('$K$'))+
  ggtitle(TeX(sprintf('Posterior distribution for the number of clusters for $\\alpha =%.3f$,$\\N =(%2.f,%2.f,%2.f, %2.f) $ ',fig8_$alpha[1],fig8_$N[1],fig8_$N[(max(fig8_$K)+1)],fig8_$N[(2*max(fig8_$K)+1)],fig8_$N[(3*max(fig8_$K)+1)])))+
  theme_minimal()+ scale_color_viridis(discrete= "TRUE", begin = 0, end = 0.9,option = "D", name = TeX(sprintf('$n$')) ,labels=unname(TeX(c(sprintf('$N$=%3.f',fig8_$N[1]),sprintf('$N$=%3.f',fig8_$N[(max(fig8_$K)+1)]),sprintf('$N$=%3.f',fig8_$N[(2*max(fig8_$K)+1)]),sprintf('$N$=%3.f',fig8_$N[(3*max(fig8_$K)+1)])))))+
...
p <- ggplot(fig8_2, aes(x=density, color =Process_type))+
  ...
  theme_minimal()+scale_color_viridis(discrete= "TRUE", name = TeX(sprintf('$n$')) ,labels=unname(TeX(c(sprintf('$N$=%3.f',fig8_2$N[1]),sprintf('$N$=%3.f',fig8_2$N[(max(fig8_2$K)+1)]),sprintf('$N$=%3.f',fig8_2$N[(2*max(fig8_2$K)+1)]),sprintf('$N$=%3.f',fig8_2$N[(3*max(fig8_2$K)+1)])))))+
  facet_wrap(~Process_type)
p
pdf(file="~/Documents/GitHub/BNPconsistency/figures/Figure8_3.pdf")
plot(p)
dev.off()
...
weights_fig <-fig8$weights 
weights_fig$n <-as.factor(weights_fig$W_val) 
p <- ggplot(weights_fig, aes(x=K, y=weights, group =K, fill = n )) + 
  geom_boxplot(alpha=0.5) +scale_fill_viridis(discrete= "TRUE", begin = 0, end = 0.9,option = "D", name = TeX(sprintf('$n$')) ,labels=unname(TeX(c(sprintf('$n$=%3.f',fig8_$N[1]),sprintf('$n$=%3.f',fig8_$N[(max(fig8_$K)+1)]),sprintf('$n$=%3.f',fig8_$N[(2*max(fig8_$K)+1)]),sprintf('$n$=%3.f',fig8_$N[(3*max(fig8_$K)+1)])))))+
  ggtitle(TeX(sprintf('Posterior distribution of the component weights $\\alpha =%.3f$,$\\N =(%2.f,%2.f,%2.f, %2.f) $ ',fig8_$alpha[1],fig8_$N[1],fig8_$N[(max(fig8_$K)+1)],fig8_$N[(2*max(fig8_$K)+1)],fig8_$N[(3*max(fig8_$K)+1)])))+
  theme_minimal()+  facet_wrap(~W_val)
p

pdf(file="~/Documents/GitHub/BNPconsistency/figures/Figure8_4.pdf")
plot(p)
dev.off()
...
df_k_20<- df_post(20, c_vec=c_vec,ind=1, post = fig8  )
df_k_200<- df_post(200, c_vec = c_vec,ind=2, post = fig8  )
df_k_2000<- df_post(2000, c_vec=c_vec,ind=3, post = fig8  )
df_k_20000<- df_post(20000,c_vec=c_vec,ind=4, post = fig8  )
...
p <- ggplot(df_fin, aes(pkn,color =Process_type)) + geom_histogram(aes(y = ..density..),binwidth = 1, alpha=0.5,position = "identity",fill='white' )+geom_vline(xintercept=3,  linetype="dashed")+
  ggtitle(TeX(sprintf('PD for the num of clusters for MTM $\\alpha =%.3f$,$\\c_vec =(%2.1f,%2.1f,%2.1f, %2.1f) $ ',fig8_$alpha[1],c_vec[1],c_vec[2],c_vec[3],c_vec[4])))+
