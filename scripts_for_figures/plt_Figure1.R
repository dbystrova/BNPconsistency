...
plt_fig1<-function(input_file, c_v =c(0.1, 0.5, 1, 2) , fig_path= "~/Documents/GitHub/BNPconsistency/figures/Figure1/" ){
...
# fig_path = "../figures/Figure1/"
 ...
 fig_df_mut <- fig_df$line%>%group_by(Process_type,N)%>%mutate(pkn =density/sum(density))
  ...
  p_l = ggplot(fig_df_mut, aes(x=K, colour = fig_df_mut$Process_type)) +
    ...
    ylab('') + ggtitle(TeX(sprintf('Posterior dist for the number of clusters for $\\N =(%3.f, %2.f, %2.f)$,$\\alpha =%.2f$, \\hat{R} =(%.3f,%.3f,%.3f,%.3f)',fig_df_mut$N[1],fig_df_mut$N[(max(fig_df_mut$K)+1)],fig_df_mut$N[(2*max(fig_df_mut$K)+1)],fig_df_mut$alpha[1],fig_df_mut$Rh[1],fig_df_mut$Rh[(max(fig_df_mut$K)+1)],fig_df_mut$Rh[(2*max(fig_df_mut$K)+1)],fig_df_mut$Rh[(2*max(fig_df_mut$K)+1)])))+
  ...
  pw <- ggplot(weights_fig_thin, aes(x=K, y=weights, group =K, fill = n )) + ylab("Weights")+xlab(TeX('$K_n$'))+scale_x_continuous(breaks = c(1,3,7,10), limits = c(0,11))+
    ...
    ggtitle(TeX(sprintf('Posterior distribution of the component weights $\\alpha =%.3f$,$\\N =(%2.f,%2.f,%2.f, %2.f) $ ',fig_df_mut$alpha[1],fig_df_mut$N[1],fig_df_mut$N[(max(fig_df_mut$K)+1)],fig_df_mut$N[(2*max(fig_df_mut$K)+1)],fig_df_mut$N[(3*max(fig_df_mut$K)+1)])))+
  ...
  pdf(paste0(fig_path,"Figure1_alpha_",fig_df_mut$alpha[1],"_3_2.pdf" ))
  plot(pw2)
  dev.off()
