...
# fig_path = "../figures/Figure1/"
 ...
 fig_df_mut <- fig_df$line%>%group_by(Process_type,N)%>%mutate(pkn =density/sum(density))
  ...
  p_l = ggplot(fig_df_mut, aes(x=K, colour = fig_df_mut$Process_type)) +
    ...
    ylab('') + ggtitle(TeX(sprintf('Posterior dist for the number of clusters for $\\N =(%3.f, %3.f, %3.f,%3.f)$,$\\alpha =%.2f$, \\hat{R} =(%.3f,%.3f,%.3f,%.3f)',n_vec[1],n_vec[2],n_vec[3],n_vec[4],fig_df_mut$alpha[1],fig_df_mut$Rh[1],fig_df_mut$Rh[(max(fig_df_mut$K)+1)],fig_df_mut$Rh[(2*max(fig_df_mut$K)+1)],fig_df_mut$Rh[(3*max(fig_df_mut$K)+1)])))+
