...
p <- ggplot(weights_fig, aes(x=K, y=weights, group =K, fill = n )) + 
  geom_boxplot(alpha=0.5) +scale_fill_viridis(discrete= "TRUE", begin = 0, end = 0.9,option = "D", name = TeX(sprintf('$n$')) ,labels=unname(TeX(c(sprintf('$n$=%3.f',fig6_$N[1]),sprintf('$n$=%3.f',fig6_$N[(max(fig6_$K)+1)]),sprintf('$n$=%3.f',fig6_$N[(2*max(fig6_$K)+1)]),sprintf('$n$=%3.f',fig6_$N[(3*max(fig6_$K)+1)])))))+
...
pdf(file="~/Documents/GitHub/BNPconsistency/figures/Figure6_4.pdf")
plot(p)
dev.off()
...
df_k_20<- df_post(20, c_vec=c_vec,ind=1, post = fig6  )
df_k_200<- df_post(200, c_vec = c_vec,ind=2, post = fig6  )
df_k_2000<- df_post(2000, c_vec=c_vec,ind=3, post = fig6  )
df_k_20000<- df_post(20000,c_vec=c_vec,ind=4, post = fig6  )
...
p <- ggplot(df_fin, aes(pkn,color =Process_type)) + geom_histogram(aes(y = ..density..),binwidth = 1, alpha=0.5,position = "identity",fill='white' )+geom_vline(xintercept=3,  linetype="dashed")+
  ggtitle(TeX(sprintf('PD for the num of clusters for MTM $\\alpha =%.3f$,$\\c_vec =(%2.1f,%2.1f,%2.1f, %2.1f) $ ',fig6_$alpha[1],c_vec[1],c_vec[2],c_vec[3],c_vec[4])))+
...
pdf(file="~/Documents/GitHub/BNPconsistency/figures/Figurefig6_6.pdf")
plot(p)
dev.off()
