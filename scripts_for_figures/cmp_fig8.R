...
pk_n_500 = MCMC_function(data_500, e0=alpha_3, K=K_nc, M=M_it, burnin=burnin_) 
pk_n_1500 = MCMC_function(data_1500, e0=alpha_3, K=K_nc, M=M_it, burnin=burnin_) 
pk_n_5000 = MCMC_function(data_5000, e0=alpha_3, K=K_nc, M=M_it, burnin=burnin_) 
pk_n_10000 = MCMC_function(data_10000, e0=alpha_3, K=K_nc, M=M_it, burnin=burnin_) 
...
df_= data.frame(K= 1:K_nc, 
                ...
                Pkn_2 = pk_n_1500,
                Pkn_3 = pk_n_5000,
                Pkn_4 = pk_n_10000)%>% gather(Process_type, density, Pkn_1:Pkn_4)

df_$alpha = c(rep(alpha_3,K_nc),rep(alpha_3,K_nc),rep(alpha_3,K_nc),rep(alpha_3,K_nc))  
df_$N = c(rep(dim(data_500$y)[1],K_nc),rep(dim(data_1500$y)[1],K_nc),rep(dim(data_5000$y)[1],K_nc),rep(dim(data_10000$y)[1],K_nc)) 
save(df_, file = "~/Documents/GitHub/BNPconsistency/saves_for_figures/cmp_fig8.RData")
