...
fig8_ <- fig8%>%group_by(Process_type,N)%>%mutate(pkn =density/sum(density))

p = ggplot(fig8_, aes(x=K, y = pkn, colour = fig8_$Process_type)) +
  geom_line(aes(x=K, y = pkn))  +ylab('') + 
  ggtitle(TeX(sprintf('Posterior distribution for the number of clusters for $\\alpha =%.3f$,$\\N =(%2.f,%2.f,%2.f) $ ',fig8_$alpha[1],fig8_$N[1],fig8_$N[(max(fig8_$K)+1)],fig8_$N[(2*max(fig8_$K)+1)])))+
