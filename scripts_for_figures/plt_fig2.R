...
p = ggplot(fig2_, aes(x=K, y = pkn, colour = fig2_$Process_type)) +
  ...
  ylab('') + ggtitle(TeX(sprintf('Posterior distribution for the number of clusters for $\\alpha =(%.3f, %.2f, %.2f)$,$\\N =%2.f$ ',fig2_$alpha[1],fig2_$alpha[(max(fig2_$K)+1)],fig2_$alpha[(2*max(fig2_$K)+1)],fig2_$N[1])))+
