...
p = ggplot(fig3_, aes(x=K, y = pkn, colour = fig3_$Process_type)) +
  ...
  ylab('') + ggtitle(TeX(sprintf('Posterior distribution for the number of clusters for $\\alpha =(%.3f, %.2f, %.2f)$,$\\N =%2.f$ ',fig3_$alpha[1],fig3_$alpha[(max(fig3_$K)+1)],fig3_$alpha[(2*max(fig3_$K)+1)],fig3_$N[1])))+
